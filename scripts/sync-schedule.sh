...
# Token an URL hängen — Separator erkennen falls checkin_url schon Query-Params hat
# (z. B. ?x-vercel-protection-bypass=... für Preview-Deployments)
if echo "$CHECKIN_URL" | grep -q '?'; then
    URL_SEP="&"
else
    URL_SEP="?"
fi
...
if [ -n "$EFFECTIVE_TOKEN" ]; then
    KIOSK_URL="${CHECKIN_URL}${URL_SEP}token=${EFFECTIVE_TOKEN}"
else
